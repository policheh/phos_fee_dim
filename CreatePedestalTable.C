...
void CreatePedestalTable(Int_t runnumber, char* file)
{
  TFile *rootfile = TFile::Open(file,"READ");
  if(!rootfile) return;
  
  char cmd[80]; sprintf(cmd,"mkdir Pedestal/%d",runnumber);
  gSystem->Exec(cmd);
  
  TFile *outfile = new TFile(Form("Pedestal/%d/PedestalTable_%d.root",runnumber,runnumber),"RECREATE");
  ...
  for(Int_t imod=0;imod<Nmod;imod++){
	...
	if (idtc==1) iDTC=15; // FEE 1 is connected to SRU port 15
        sprintf(name,"Pedestal/%d/Pedestal_%d_SRUM%d-%d_DTC%d.txt",runnumber,runnumber,imod,isru,iDTC);
